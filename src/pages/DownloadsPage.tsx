...
export default function DownloadsPage() {
  ...
  return (
    <Box sx={{ pb: 2 }}>
      <SEOHead title="My Downloads" noIndex />
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, p: 2 }}>
        <IconButton onClick={() => navigate(-1)} aria-label="Go back">
          <ArrowBack />
        </IconButton>
        <Typography variant="h6" fontWeight={700}>Downloads</Typography>
      </Box>
      ...
      {loading ? (
        ...
        <Box sx={{ textAlign: 'center', py: 8 }}>
          <DownloadOutlined sx={{ fontSize: 64, color: 'text.disabled', mb: 2 }} />
          <Typography variant="h6" color="text.secondary">No downloads yet</Typography>
